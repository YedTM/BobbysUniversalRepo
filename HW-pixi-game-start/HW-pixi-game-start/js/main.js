...
function setup() {
	...
	// #7 - Load sprite sheet
	explosionTextures = loadSpriteSheet(1);
    smallerExplosionTextures = loadSpriteSheet(3);
	// #8 - Start update loop
...
function gameLoop(){
    ...
    let amt = 8 * dt; // at 60 FPS would move about 10% of distance per update
	...
	for (let c of circles){
        ...
        if (c.isAlive && rectsIntersect(c,ship)){
            hitSound.play();
            createExplosion(c.x,c.y,64,64, smallerExplosionTextures);
            gameScene.removeChild(c);
            c.isAlive = false;
            decreaseLifeBy(20);
        }
...
function fireBullet(e){
    ...
    else
    {
        if (bullets.length <= 30)
        {
            for (let i = 0; i < 3; i++)
            {
                let b = new Bullet(0xFFFFFF,ship.x + 15 - (i * 15),ship.y);
                bullets.push(b);
                gameScene.addChild(b);
            }
...
function loadSpriteSheet(rowNumber){
    ...
    for (let i=0; i<numFrames;i++){
        ...
        let frame = new PIXI.Texture(spriteSheet, new PIXI.Rectangle(i*width, 64 * rowNumber, width, height));
        textures.push(frame);
    }
    return textures;
}
...
function createExplosion(x, y, frameWidth, frameHeight, textures) {
    ...
    let expl = new PIXI.AnimatedSprite(textures);
    expl.x = x -w2; //we want the explosions to appear at the center of the cirle
    expl.y = y - h2; //ditto
